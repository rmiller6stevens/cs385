/*******************************************************************************
 * Name        : rbtree.h
 * Author      : Robert Miller, Burak Yesil
 * Version     : 1.0
 * Date        : 11/16/21
...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        size_++;
            ...
            //the children are allready set made to nullptr
            //the color is already set to red
            insert_fixup(z);
    }
...
private:
    ...
    void delete_tree(Node<K, V> *n) {
        ...
        if(n->left == nullptr && n->left == nullptr){
            delete n;
            return;
        } 
        if(n->left != nullptr){
     ...
     * finds the lowest node first then goes back up, adding 1 for each level starting at the lowest leaf node
     */
    int height(Node<K, V> *node) const {
        ...
        count++;
        count+= std::max(height(node->left), height(node->right));
        return count;
    }
    ...
    int diameter(Node<K, V> *node) const {
        ...
        int height_total = 2 + height(node->left) + height(node->right);
        int temp = std::max(diameter(node->left), diameter(node->right));
        return std::max(temp, height_total);
    }
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const {
        ...
        if(node->left != nullptr){
            count += sum_levels(node->left, level + 1);
        }
        if(node->right != nullptr){
            count += sum_levels(node->right, level + 1);
        }
        return count;
    }
    ...
    size_t sum_null_levels(Node<K, V> *node, size_t level) const {
        ...
        if(node->left != nullptr && node->right == nullptr){
            level++;
            count += level;
            count += sum_null_levels(node->left, level);
        }
        if(node->left == nullptr && node->right != nullptr){
            level++;
            count += level;
            count += sum_null_levels(node->right, level);
        }
        if(node->left != nullptr && node->right != nullptr){
            level++;
            count += sum_null_levels(node->left, level);
